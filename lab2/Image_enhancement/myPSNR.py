...
def myPSNR( orig_image, approx_image ):
    ...
    # convert from uint8 to float, otherwise calculation will be off due to overflow/precision errors
    orig_image = np.asarray(orig_image, dtype=np.float64)
    approx_image = np.asarray(approx_image, dtype=np.float64)
